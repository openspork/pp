...
def get_group_groups(group, relationship):
    if relationship == "parents":
        ...
        active_groups = (
            ...
            .where(GroupGroups.parent == group).order_by(Group.name)
        )
...
def remove_groups_from_phone(prev_group_ids, phone):
    for prev_group_id in prev_group_ids:
        ...
        delete_query = PhoneGroups.delete().where(
            (PhoneGroups.phone == phone) & (PhoneGroups.group == group)
        )
...
def remove_groups_from_group(prev_group_ids, group, relationship):
    if relationship == "children":
        for child_group_id in prev_group_ids:
            ...
            delete_query = GroupGroups.delete().where(
                (GroupGroups.parent == group) & (GroupGroups.child == child_group)
            )
            delete_query.execute()
    elif relationship == "parents":
        for parent_group_id in prev_group_ids:
            ...
            delete_query = GroupGroups.delete().where(
                (GroupGroups.child == group) & (GroupGroups.parent == parent_group)
            )
            delete_query.execute()
